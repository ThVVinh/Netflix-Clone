...
for movie_id in movieIds:
    ...
    for result in response["results"]:
        if result.get("type", "") == "Trailer" and result.get("site", "") == "YouTube" and result.get("official", False):
            youtube_base = "https://www.youtube.com/watch?v="
            youtube_key = result.get("key", "")
            quality = result.get("size", 0)
            sql = f"""
            INSERT INTO [dbo].[media] (movie_id, type, url)
            VALUES ({movieIds.index(movie_id) + 1}, 'Trailer', '{youtube_base}{youtube_key}'); {quality}
            """
            print(sql)
            break